...
public sealed class SortDirectiveParser<T> : ISortDirectiveParser<T>
{
    /// <inheritdoc/>
    public IEnumerable<ISortCriterion<T>> Parse(string directives)
    {
        ...
        if (parts.Any(string.IsNullOrEmpty))
        {
            throw new FormatException(
                "Invalid sorting string format: " +
                "The string contains an empty sorting directive.");
        }
    ...
    private static ISortCriterion<T> ParseDirective(string directive)
    {
        ...
        var direction = parts.Length switch
        {
            ...
            _ => throw new FormatException(
                $"Invalid format for sorting directive: '{directive}'. " +
                $"Expected format is 'Property [ASC|DESC]'.")
        };
        ...
        {
            throw new FormatException(
                "Failed to parse a sorting directive from the input string. " +
    ...
    private static SortDirection ParseDirection(string direction) =>
        direction.ToUpperInvariant() switch
        {
            ...
            _ => throw new FormatException(
                $"Invalid sort direction in sorting directive: '{direction}'. " +
